...
TEST(Plane, DistanceToPointForXZGroundPlane) {
    // Y-up ground plane through the origin.
...
TEST(Plane, ConstructFromNormalAndPoint) {
    ...
    EXPECT_NEAR(p.DistanceToPoint(Vec3(100.0f, 5.0f, -50.0f)), 0.0f, kEps); // any point at y=5
...
TEST(Plane, ConstructFromThreePointsGivesCorrectNormalAndDistance) {
    // CCW triangle in the XZ plane (viewed from +Y) should give normal +Y.
...
TEST(Plane, NormalizedPreservesGeometryWithUnscaledNormal) {
    Plane p(Vec3(0.0f, 2.0f, 0.0f), -10.0f); // un-normalized: |normal|=2
    Plane n = p.Normalized();
    EXPECT_NEAR(n.normal.Length(), 1.0f, kEps);
    // Same plane, so distance to any given point should be unchanged.
...
TEST(Plane, IntersectRayMissesParallelRay) {
    Plane p(Vec3(0.0f, 1.0f, 0.0f), 0.0f);
    Ray r(Vec3(0.0f, 5.0f, 0.0f), Vec3(1.0f, 0.0f, 0.0f)); // parallel to the plane
    float t;
